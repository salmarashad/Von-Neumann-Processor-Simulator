...
bool CheckStall(int opcodeFirst, int Rs, int Rt, int Rd);
...
void decode(InstData *instruction)
{
    ...
    if (Stages[2] != NULL)
    {
        ...
        if (CheckStall((*Stages[2]).opcode, (*Stages[1]).src1, (*Stages[1]).src2, (*Stages[2]).dest))
        {
            printf("I'm stalling DUE TO the one directly before me %d \n", (*Stages[2]).pc);
            stallTEMP = 1;
        }
    }
    else if (Stages[4] != NULL) // wb -->  decode
    {

        if (CheckStall((*Stages[4]).opcode, (*Stages[1]).src1, (*Stages[1]).src2, (*Stages[4]).dest))
        {
            printf("im forwarding from the one directly before before me %d \n", (*Stages[4]).pc);
            forward = 1;
        }
    }
...
bool CheckStall(int opcodeFirst, int Rs, int Rt, int Rd)
{
    ...
    case 9:
        RegWrite = 1;
        sameSourceDest = (Rs == Rd || Rd == Rt) && Rd != 0;
        break;
    case 10:
        RegWrite = 1;
        sameSourceDest = Rs == Rd && Rd != 0;
        break;
    default:
        RegWrite = 0;
        break;
    ...
    return sameSourceDest && RegWrite;
}
void execute(InstData *instruction)
{
    ...
    if (forward == 1)
    {
        ...
        (*instruction).valuesrc2 = Registers[(*instruction).src2];
        (*instruction).destValue = Registers[(*instruction).dest];
        printf("Im forwarding to execute \n");
        forward = 0;
    }
    ...
    case 4: // branch
        ...
        if ((*instruction).ALUoutput != 0)
        {
            ...
            {
                printf(" Im branching and skipping one isntruction only \n");
            }
    ...
    case 7: // jump
        ...
        else if ((*instruction).ALUoutput - (*instruction).pc > 2)
        {
            ...
            // printf("i'm jumping to address %d \n",pc);
        }
...
void memAccess(InstData *instruction)
{
    ...
    switch ((*instruction).opcode)
    {
    case 11: // write to memory from the srcReg
    ...
    case 10:
        ...
        // displayMemory();
        // printf(" address %d", (*instruction).ALUoutput);
...
void StartComputer()
{
    ...
    printf(" I HAVE IN THE MEMORY A TOTAL OF %d INSTRUCTIONS \n", memoryCounter);
